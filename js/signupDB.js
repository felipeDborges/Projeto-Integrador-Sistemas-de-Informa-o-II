document.getElementById('signupForm').addEventListener('submit', function (event) {
    ...
    request.onsuccess = function (event) {
        ...
        addRequest.onsuccess = function (event) {
            ...
            let goBack = window.confirm('Dados do usuário cadastrados com sucesso.');
            if (goBack) window.location.href = 'login.html'
        };

        addRequest.onerror = function (event) {
            console.log("Erro ao cadastrar dados do usuário.");
            let goBack = window.confirm('Usuario já cadastrado.');
            if (goBack) window.location.href = 'login.html'
        };
    };
});
